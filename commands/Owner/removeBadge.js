...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        const msgEmbed = new MessageEmbed()
            .setTitle("San Andreas Life | Badge Removed")
...
Member: <@!${mentionedUser.user.id}>
        ...
        const loggingEmbed = new MessageEmbed()
            .setTitle("San Andreas Life | Badge Removed Logs")
...
The system has failed. Make sure that you mentioned a user by @ / ID & That the name of the badge is correct.
        ...
        if (badge == "MediaMember") {
            ...
            await db.set(`profile.rank.media.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else if (badge == "Developer") {
            message.channel.send(msgEmbed)
            await db.set(`profile.rank.dev.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else if (badge == "Staff") {
            message.channel.send(msgEmbed)
            await db.set(`profile.rank.staff.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else if (badge == "DirectorOfStaff") {
            message.channel.send(msgEmbed)
            await db.set(`profile.rank.dos.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else if (badge == "Management") {
            message.channel.send(msgEmbed)
            await db.set(`profile.rank.man.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else if (badge == "Director") {
            message.channel.send(msgEmbed)
            await db.set(`profile.rank.dir.${mentionedUser.user.id}`, `<:ERROR:902121535668637727>`)
            loggingChannel.send(loggingEmbed)
        } else {
